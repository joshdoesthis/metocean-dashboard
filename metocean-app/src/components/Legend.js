...
const Legend = (props) => {
  ...
  const currentUnit = selectedUnit.scale ? selectedUnit.scale : defaultUnit;

  const scales = {
    kts: [
      { scale: 'kts', ratio: 1 },
      { scale: 'mph', ratio: 1.15078 },
      { scale: 'km/h', ratio: 1.852 },
      { scale: 'm/s', ratio: 0.514444 },
      { scale: 'ft/s', ratio: 1.68781 },
    ],
  };
      ...
      <div style={styles.unit}>
        <div
          ...
          onClick={() => {
            ...
            const nextUnitIndex =
              scales[defaultUnit].findIndex(
                ({ scale }) => scale == currentUnit
              ) + 1;

            const nextSelectedUnit =
              scales[defaultUnit][
                nextUnitIndex < numScales ? nextUnitIndex : 0
              ];
